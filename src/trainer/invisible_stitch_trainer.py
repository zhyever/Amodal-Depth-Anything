...
class InvisibleStitchTrainer:
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                with self.accelerator.accumulate(self.model):
                    ...
                    pred = self.model(
                        rgb, 
                        invisible_mask=batch["invisible_mask"].float().to(device),
                        observation=batch['depth_observation'].float().to(device)) # both: from -1 to 1
            
                    # gt is in the range of [0, 1]
                    pred = F.interpolate(pred, size=(depth_gt.shape[2], depth_gt.shape[3]), mode='nearest')

                    if torch.isnan(pred).any():
                        logging.warning("pred contains NaN.")

                    # Get the target for loss depending on the prediction type
                    if self.gt_mask_type is not None:
                        entire_mask = batch['guide'].to(device)
                        entire_mask = F.interpolate(entire_mask.float(), size=(pred.shape[2], pred.shape[3])) > 0
                            
                        invisible_mask = batch['invisible_mask'].to(device)
                        invisible_mask = F.interpolate(invisible_mask.float(), size=(pred.shape[2], pred.shape[3])) > 0
                        
                        visible_mask = batch['visible_mask'].to(device)
                        visible_mask = F.interpolate(visible_mask.float(), size=(pred.shape[2], pred.shape[3])) > 0
                        
                        if self.cfg.trainer.loss_stategy == 'invisible_part':
                            invisible_mask = torch.logical_and(valid_mask, invisible_mask)
                            latent_loss = self.loss(
                                pred[invisible_mask].float(),
                                depth_gt[invisible_mask].float())
                        
                        elif self.cfg.trainer.loss_stategy == 'entire_target_object':
                            entire_mask = torch.logical_and(valid_mask, entire_mask)
                            latent_loss = self.loss(
                                pred[entire_mask].float(),
                                depth_gt[entire_mask].float())
                            
                        elif self.cfg.trainer.loss_stategy == 'ssi invisible_part':
                            try:
                                _, scale, shift = align_depth_least_square(
                                    gt_arr=depth_gt.detach().cpu().numpy(),
                                    pred_arr=pred.detach().cpu().numpy(),
                                    valid_mask_arr=visible_mask.detach().cpu().numpy(),
                                    return_scale_shift=True,
                                    max_resolution=None)
                                depth_align = pred * torch.tensor(scale).cuda() + torch.tensor(shift).cuda()
                                invisible_mask = torch.logical_and(valid_mask, invisible_mask)
                                latent_loss = nn.functional.l1_loss(
                                    depth_align[invisible_mask].float(),
                                    depth_gt[invisible_mask].float())
                            except np.linalg.LinAlgError as e:
                                logging.error(f'LinAlgError: {e}')
                                latent_loss = torch.sum(pred) * 0
                            if torch.isnan(latent_loss).any():
                                logging.warning("latent_loss contains NaN.")
                                latent_loss = torch.sum(pred) * 0
                            
                        elif self.cfg.trainer.loss_stategy == 'ssi entire_target_object': # make more sense to me
                            try:
                                _, scale, shift = align_depth_least_square(
                                    gt_arr=depth_gt.detach().cpu().numpy(),
                                    pred_arr=pred.detach().cpu().numpy(),
    ...
    def validate_single_dataset(
        ...
        for i, batch in pbar:
            ...
            pred = self.model(
                        rgb, 
                        invisible_mask=batch["invisible_mask"].float().to(self.device),
                        observation=batch['depth_observation'].float().to(self.device))
