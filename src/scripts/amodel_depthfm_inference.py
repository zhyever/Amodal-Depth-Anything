...
if __name__ == "__main__":
    ...
    output_path = './work_dir/project_folder/cvpr_depthfm/depthfm_results'
    ...
    for image_file in tqdm(valid_samples):
        ...
        depth_pred_ts = model(
                        ims=input_image.unsqueeze(dim=0), 
                        num_steps=4, 
                        ensemble_size=1, 
                        mode='infer', 
                        depth=None, 
                        rand_num_generator=generator,
                        guide_mask=whole_mask.unsqueeze(dim=0).float().cuda(),  # 0-1
                        guide_rgb=None, # -1-1
                        observation=observation_depth.unsqueeze(dim=0).float().cuda()) # 0-1

        depth_pred_ts = F.interpolate(depth_pred_ts, size=resize_to_hw, mode='bilinear', align_corners=False)
